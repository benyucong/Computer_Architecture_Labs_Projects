...
int mul_matrix(matrix *result, matrix *mat1, matrix *mat2)
{
    ...
    for (int x = 0; x < mat2_transposed->rows; ++x)
    {
        ...
        {
            // Can utilize block matrix to speed up, but not necessary, I'm lazy, lol!
...
int pow_matrix(matrix *result, matrix *mat, int pow)
{
    ...
    {
        matrix *mat1 = NULL;
        allocate_matrix(&mat1, mat->rows, mat->cols);
        pow_matrix(mat1, mat, pow >> 1);
        mul_matrix(result, mat1, mat1);
        deallocate_matrix(mat1);
    }
    else
    {
        matrix *mat1 = NULL, *tmp_res = NULL;
        allocate_matrix(&mat1, mat->rows, mat->cols);
        allocate_matrix(&tmp_res, mat->rows, mat->cols);
        pow_matrix(mat1, mat, pow >> 1);
        mul_matrix(tmp_res, mat1, mat1);
        mul_matrix(result, tmp_res, mat);
        deallocate_matrix(mat1);
        deallocate_matrix(tmp_res);
    }
    return 0;
}
